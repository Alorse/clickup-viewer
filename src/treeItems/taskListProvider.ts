    ...
    implements vscode.TreeDataProvider<vscode.TreeItem>
{
    ...
    async getChildren(element?: Space): Promise<vscode.TreeItem[]> {
        ...
        if (element instanceof TeamItem) {
            const spaces: Space[] = await this.itemsController.getSpaces(
                element.id,
            );
        ...
        if (element instanceof SpaceItem) {
            const folders: Folder[] = await this.apiWrapper.getFolders(
                element.id,
            );
            resolve = Object.values(folders).map((folder: Folder) => {
                return new FolderItem(folder, this.collapsedConst);
            });
            const lists: List[] = await this.apiWrapper.getFolderLists(
                element.id,
            );
            await Promise.all(
                Object.values(lists).map(async (list: List) => {
                    ...
                    resolve.push(
                        new ListItem(list, this.collapsedConst, taskCount),
                    );
                }),
            );
        }
        ...
        if (element instanceof FolderItem) {
            const lists: List[] = await this.apiWrapper.getLists(
                element.folder.id,
            );
            await Promise.all(
                Object.values(lists).map(async (list: List) => {
        ...
        if (element instanceof ListItem) {
            const tasks: Task[] = await this.apiWrapper.getTasks(
                element.list.id,
            );
