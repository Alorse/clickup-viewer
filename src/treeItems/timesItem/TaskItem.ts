...
export class TaskItem extends TreeItem {
    constructor(
        ...
        const startTimes = TimeItem.start.map((start) => unixtimeToString(Number.parseInt(start)));
        const endTimes = (TimeItem.end ?? []).map((end) => unixtimeToString(Number.parseInt(end)));
        ...
        this.tooltip = `Task: ${TimeItem.task.custom_id ?? TimeItem.task.id} : ${TimeItem.task.name}\n` +
            TimeItem.start.map((start, index) => 
